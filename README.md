...
Dictation takes no selection: hold ⌃⌥⌘Space, speak, release, and the text is pasted where the cursor was — or tap it, and it listens hands-free until the next press. Recognition is Apple's own and runs on the Mac; the chosen model then punctuates the transcript and drops the hesitations, unless that model is **Raw**, which pastes it as it was heard. Whatever is playing pauses while it listens, so music doesn't talk over the voice, and resumes once the microphone closes. A vocabulary in Settings keeps names and jargon spelled your way (`heard → written` replacements, plus the terms passed to recognition and cleanup). A second shortcut listens in another language, each shortcut picks what its dictation becomes — cleaned up, translated to English, or turned into a prompt, all in that same single call — and the last 50 dictations are listed in Settings, text only.
...
- **Dictation** ([Speech/](Sources/Claudio/Speech), [Coordinator/DictationCoordinator.swift](Sources/Claudio/Coordinator/DictationCoordinator.swift)): the key going down opens the microphone, the key coming up closes it — or, after a tap, the next press does. Recognition is Apple's, on-device (`SpeechAnalyzer` on macOS 26, `SFSpeechRecognizer` pinned to the device below), behind a `SpeechEngine` protocol so the cycle is tested on a fake one. Only the transcript leaves the Mac, sent to the chosen model for cleanup — nothing at all with Raw or a local model. A failed cleanup still pastes the transcript. Playback is paused through the private MediaRemote framework and resumed only if Claudio paused it: since macOS 15.4 only Apple's own binaries are told what is playing, so the state is read through `osascript` in a child process that the microphone never waits for, while the pause and play commands still go out from the app.
