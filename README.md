...
- **Dictation** ([Speech/](Sources/Claudio/Speech), [Coordinator/DictationCoordinator.swift](Sources/Claudio/Coordinator/DictationCoordinator.swift)): the key going down opens the microphone, the key coming up closes it — or, after a tap, the next press does. Recognition is Apple's, on-device (`SpeechAnalyzer` on macOS 26, `SFSpeechRecognizer` pinned to the device below), behind a `SpeechEngine` protocol so the cycle is tested on a fake one. Only the transcript leaves the Mac, sent to the chosen model for cleanup — nothing at all with Raw or a local model. A failed cleanup still pastes the transcript. Other apps are muted by a Core Audio process tap whose audio is never read, so it works on any output, a USB interface included, and asks for no permission; a launch removes the tap a crash left behind.
