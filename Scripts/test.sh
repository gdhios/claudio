...
# free instruction, dictation listening and cleaning up), palette (plain and
# filtered), Settings (API key, engine per action, local server, shortcuts,
# dictation).
for mode in panel panel-streaming panel-error panel-free panel-listening panel-dictation-cleaning palette palette-filtre settings settings-prompts settings-ollama settings-shortcuts settings-dictation; do
    preview_shot "$mode"
done
